...
export default function GameSetup({
  ...
  // Pull dynamic aesthetic tokens based on whichever category is chosen
  ...
  return (
    <div className={`w-full max-w-2xl bg-slate-800/80 border ${currentTheme.borderColor} p-6 md:p-8 rounded-3xl shadow-2xl space-y-8 backdrop-blur-md transition-colors duration-500`} >
      ...
      <div className="space-y-3">
        <label className="text-xs font-bold uppercase tracking-widest text-slate-500 block"> Choose Category </label>
          ...
          {CATEGORIES.map((cat: IGameCategory) => {
            ...
            return (
              <button 
                ...
                className={`p-4 text-sm font-bold text-left rounded-2xl border transition-all duration-300 transform active:scale-[0.98] cursor-pointer flex items-center justify-between group ${ isSelected ? `bg-slate-900 border-cyan-500 shadow-lg ${catTheme.focusRing}` : "bg-slate-900/40 border-slate-700/60 hover:border-slate-600 text-slate-300 hover:text-white" }`} 
              >
                <span>{cat.name}</span>
                {isSelected && (
      ...
      <div className="space-y-3">
        <label className="text-xs font-bold uppercase tracking-widest text-slate-500 block"> Select Difficulty </label>
          ...
          {["easy", "medium", "hard"].map((diff) => {
            ...
            return (
              <button 
                ...
                className={`py-3 text-xs font-black uppercase tracking-widest rounded-xl border transition-all duration-300 capitalize cursor-pointer ${ isSelected ? currentTheme.difficultyActive : "bg-slate-900/40 border-slate-700/60 hover:border-slate-600 text-slate-400 hover:text-slate-200" }`} 
              >
                {diff}
              </button>
            );
