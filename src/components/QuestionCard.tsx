...
// ✨ Native browser helper to decode HTML entities securely without external libraries
...
export default function QuestionCard({
  ...
  return (
    <div
      className={`w-full max-w-2xl bg-slate-800/80 border ${currentTheme.borderColor} p-6 md:p-8 rounded-3xl shadow-2xl space-y-6 backdrop-blur-md transition-all duration-500`}
    >
        ...
        {options.map((option: string) => {
          ...
          return (
            <button
              ...
              className={`w-full p-4 rounded-xl text-left text-sm font-semibold transition-all border group cursor-pointer active:scale-[0.99] min-h-[56px] flex items-center justify-between ${
                isSelected
                  ? `bg-slate-900 border-cyan-500 text-cyan-400 font-bold shadow-lg ${currentTheme.focusRing}`
            ...
            >
              ...
              {isSelected && (
                <span
                  className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded ${currentTheme.badge}`}
                >
                  Selected
                </span>
              )}
...
// import { ICleanedQuestion } from "@/models/Question";
// import { decode } from "html-entities"; // ✨ Imported safe decoding utility
...
//       {/* Dynamic Themed Question Header String */}
//       <h3 className="text-lg md:text-xl font-bold text-slate-100 leading-relaxed text-center min-h-[60px] flex items-center justify-center">
//         {decode(question.questionText)} {/* ✨ Replaced dangerouslySetInnerHTML */}
//       </h3>
...
//               }`}
//             >
//               <span>{decode(option)}</span> {/* ✨ Replaced dangerouslySetInnerHTML */}
