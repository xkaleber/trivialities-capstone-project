...
export default function DashboardPage() {
  ...
  // Create chronological array for the chart visualization (oldest to newest match)
  const chartData = [...history].reverse().slice(-7); // Target up to the last 7 matches
        ...
        <div className="bg-slate-800/40 p-6 rounded-2xl border border-slate-700/40 shadow-xl space-y-4">
          ...
          <div className="h-48 flex items-end justify-around gap-2 pt-6 border-b border-slate-700 px-4 bg-slate-950/30 rounded-xl">
            {chartData.map((game: any, index) => {
              // Map score max output parameters out of 100 points scale
              const barHeightPercentage = Math.min(
                Math.max((game.score / 100) * 100, 8),
                100,
              );
              return (
                <div
              ...
              <table className="w-full text-left border-collapse text-sm">
                ...
                <tbody className="divide-y divide-slate-800/40">
                  {leaderboard.map((player: any) => (
                    <tr
                      key={player.rank}
                      className={`hover:bg-slate-800/40 transition-colors ${player.playerName === session?.user?.name ? "bg-cyan-950/20" : ""}`}
                    >
